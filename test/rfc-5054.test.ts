...
import {Client, computeVerifier, PARAMS, Server} from "../src";
...
describe('rfc-5054', () => {
  ...
  before(() => {
    v = computeVerifier(params, s, I, P);
  });

  it('x', () => {
    const client = new Client(params, s, I, P, a);
    assert.equal(asHex(client.data.x), x_expected);
  });
  ...
  it('k', () => {
    const client = new Client(params, s, I, P, a);
    assert.equal(asHex(client.data.k), k_expected);
  });
  ...
  it('B', () => {
    const server = new Server(params, v, b);
  ...
  it('u', () => {
    ...
    assert.equal(asHex(<BigInteger>client.data.u), u_expected);
  });

  it('S client', () => {
    const client = new Client(params, s, I, P, a);
    client.setB(Buffer.from(B_expected, 'hex'));
    assert.equal((<Buffer>client.data.S).toString('hex'), S_expected);
  });

  it("S server", () => {
    const server = new Server(params, v, b);
    server.setA(Buffer.from(A_expected, 'hex'));
    assert.equal((<Buffer>server.data.S).toString('hex'), S_expected);
  });
});
